...
| Action_equal( a_Action , a_Action) = true 
| Action_equal( b_Action( x1)  , b_Action( y1) ) = true  andalso Bool_equal( x1 , y1) 
...
linearize ( [ def ( rv ( Init_RecName ,    [  ] ) , n_deploy(l_Loc,p_prefix(b_Action ( true_Bool ),p_nil)) ) 
 ] ) 
 ; 
