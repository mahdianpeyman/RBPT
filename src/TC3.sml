...
(* Msg_Type_equal function *)
 ...
 | Msg_Type_equal (reply_Msg_t , reply_Msg_t ) = true
...
fun  isType ( ack_Msg_t , ack_Msg) = true
 |  isType ( reply_Msg_t , reply_Msg(x0)) = true
 |  isType(x,y) = false
 ; 
...
fun Loc_equal( unknown_Loc , unknown_Loc) = true 
 | Loc_equal( l_Loc , l_Loc) = true 
| Loc_equal (x,y) = false 
;

 (* equal function for Sort  <Msg> :*)
fun Msg_equal( ack_Msg , ack_Msg) = true 
 | Msg_equal( reply_Msg( x1)  , reply_Msg( y1) ) = true  andalso Nat_equal( x1 , y1) 
| Msg_equal (x,y) = false 
;
...
linearize ( [ def ( rv ( Test_RecName ,    [ Bool_Var (z_1 )  , Nat_Var (w_1 )  ] ) , p_choice(p_prefix(snd_Action ( ack_Msg ),p_cond ( z_1 , p_prefix(snd_Action ( reply_Msg ( w_1 ) ),p_nil) , p_nil )),p_sum ( Nat_Var(t_2) , p_prefix(rcv_Action ( reply_Msg ( t_2 ) ),p_prefix(snd_Action ( ack_Msg ),p_nil)))) ) 
 , def ( rv ( Init_RecName ,    [  ] ) , n_deploy(l_Loc,p_recvar ( rv ( Test_RecName ,  [ Bool_Var (true_Bool) , Nat_Var (succ_Nat ( zero_Nat )) ] )  )) ) 
 ] ) 
 ; 
